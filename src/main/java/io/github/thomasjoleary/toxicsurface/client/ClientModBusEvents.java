...
public final class ClientModBusEvents {
    ...
    /** The depth-reconstruction shader behind {@link ToxicGasFogRenderer}'s per-pixel gas haze. */
    @SubscribeEvent
    public static void registerShaders(RegisterShadersEvent event) {
        try {
            event.registerShader(
                    ...
                    shader -> ToxicGasFogRenderer.shader = shader);
        } catch (IOException e) {
            ToxicSurface.LOGGER.error("Failed to compile the toxic-fog shader; the gas haze will be disabled", e);
        }
    }
}
