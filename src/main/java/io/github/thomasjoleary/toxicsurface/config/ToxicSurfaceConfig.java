...
public final class ToxicSurfaceConfig {
    ...
    static {
        ...
        b.push("timing");
        TIME_TO_TOXIC_TICKS = b.comment(
                        "In-game ticks before the surface first turns toxic (120000 ~= 5 in-game days).")
                .defineInRange("timeToToxicTicks", 120_000, 0, Integer.MAX_VALUE);
        TOXIC_START_Y =
                b.comment("Initial toxic ceiling Y (sea level by default).").defineInRange("toxicStartY", 63, -64, 320);
        ESCALATION_SPEED_PER_DAY = b.comment("Blocks the toxic ceiling rises per in-game day (0 = static line).")
        ...
        b.push("air_bar");
        AIR_BAR_DRAIN_TICKS = b.comment(
                        "Ticks for the toxic air bar to drain full->empty when unprotected (300 = 15s).")
                .defineInRange("airBarDrainTicks", 300, 1, 12_000);
        AIR_BAR_REFILL_TICKS = b.comment("Ticks for the toxic air bar to refill empty->full in clean/protected air.")
                .defineInRange("airBarRefillTicks", 60, 1, 12_000);
        TOXIC_DAMAGE_PER_SECOND = b.comment("Real damage per second once the air bar empties (kills the player).")
                .defineInRange("toxicDamagePerSecond", 2.0, 0.0, 1_000.0);
        NAUSEA_WHILE_DRAINING = b.comment("Apply Nausea while the air bar is draining (not after).")
                .define("nauseaWhileDraining", true);
        b.pop();

        b.push("sludge");
        SLUDGE_DAMAGE = b.comment("Contact damage dealt by toxic sludge per interval.")
                .defineInRange("sludgeDamage", 2.0, 0.0, 1_000.0);
        SLUDGE_INTERVAL_TICKS = b.comment("Ticks between sludge contact-damage applications (10 = 0.5s).")
                .defineInRange("sludgeIntervalTicks", 10, 1, 1_200);
        SLUDGE_DEPTH_MIN = b.comment("Starting sludge band depth below the water surface (thin skin).")
                .defineInRange("sludgeDepthMin", 4, 0, 384);
        SLUDGE_DEPTH_MAX = b.comment("Sludge band depth when the toxic line reaches escalationMaxY.")
                .defineInRange("sludgeDepthMax", 24, 0, 384);
        WATER_CONVERSION_MODE = b.comment("BANDED = surface-anchored depth band (DESIGN.md §2b); FULL = whole column.")
                .defineEnum("waterConversionMode", WaterConversionMode.BANDED);
        b.pop();

        b.push("equipment");
        MASK_DURATION_TICKS = b.comment("Active filter time for a face mask (2400 = 2 minutes).")
                .defineInRange("maskDurationTicks", 2_400, 1, 1_728_000);
        MASK_TICK_MODE = b.comment("IN_GAS_ONLY = filter only counts down in toxic gas; ALWAYS = always ticks.")
                .defineEnum("maskTickMode", MaskTickMode.IN_GAS_ONLY);
        SUIT_FILTER_CAPACITY =
        ...
        b.push("world");
        AFFECTED_DIMENSIONS = b.comment("Dimensions that turn toxic (opt-in whitelist; Overworld only by default).")
                .defineList(
                        "affectedDimensions",
                        List.of("minecraft:overworld"),
                        () -> "minecraft:overworld",
                        o -> o instanceof String);
        FOLIAGE_DECAY_BLOCKS_PER_TICK = b.comment("Global throttle for the foliage death pass (blocks/tick).")
                .defineInRange("foliageDecayBlocksPerTick", 64, 1, 65_536);
        TOXIC_RAIN_ENABLED = b.comment("Enable the surface toxic-rain overlay.").define("toxicRainEnabled", true);
        TOXIC_RAIN_DECAY_MULTIPLIER = b.comment("Foliage decay speed-up while toxic rain is falling.")
                .defineInRange("toxicRainDecayMultiplier", 2.0, 1.0, 100.0);
        b.pop();

        b.push("performance");
        ENCLOSURE_FLOOD_FILL_BUDGET = b.comment(
                        "Max air blocks an enclosure flood-fill visits before declaring 'open' (DESIGN.md §8).")
                .defineInRange("enclosureFloodFillBudget", 4_096, 64, 1_048_576);
        WATER_CONVERSION_BLOCKS_PER_TICK = b.comment(
                        "Global throttle for the water->sludge conversion queue (blocks/tick).")
